misha_champ_import <- function (directory = getwd(), offset = 100, arraytype = "450K") 
{
  message("[===========================]")
  ...
  GreenMean <- do.call(cbind, lapply(G.idats, function(xx) xx$Quants[commonAddresses, 
                                                                     "Mean"]))
  RedMean <- do.call(cbind, lapply(R.idats, function(xx) xx$Quants[commonAddresses, 
                                                                   "Mean"]))
  ...
  control_probe <- rownames(Anno$ControlProbe)[which(Anno$ControlProbe[, 
                                                                       1] == "NEGATIVE")]
